...
export const MatchCard = ({ match, onClick, activeRankingData, historyData, masterTeams = [] }: MatchCardProps) => {
  ...
  // ------------------------------------------------
  // 2. [Helper] 랭킹 뱃지 (Relative 위치로 변경)
  // ------------------------------------------------
  const getRankBadge = (rank?: number) => {
    ...
    return (
      <span className={`px-1.5 py-[1px] rounded-[4px] border text-[8px] shadow-sm leading-none ${colorClass}`}>
        R.{rank}
      </span>
    );
  };
  ...
  return (
    <div 
      ...
      className={`relative bg-slate-950 p-3 rounded-xl border ${isCompleted ? 'border-slate-800' : 'border-slate-700'} hover:border-emerald-500 cursor-pointer shadow-md group transition-all duration-200 hover:scale-[1.01]`}
    >
        {/* 매치 라벨 */}
        <div className="flex justify-center items-center mb-3 relative">
            <span className="text-[9px] font-bold text-slate-500 bg-slate-900 px-2 py-0.5 rounded uppercase border border-slate-800">
        ...
        <div className="flex justify-between items-start px-1">
            ...
            <div className="flex flex-col items-center w-[35%] gap-1">
                {/* 1. 엠블럼 (가림 없음) */}
                <img src={match.homeLogo} className="w-12 h-12 rounded-full bg-white object-contain p-1 shadow-lg mb-1" alt="" onError={(e)=>{e.currentTarget.src=FALLBACK_IMG}}/>
                
                {/* 2. 🔥 [수정] 상태바 (랭킹 + 컨디션) 하단 배치 */}
                ...
                <div className="text-center w-full mt-1">
                    <span className="text-xs font-bold text-white leading-tight truncate w-full block">{match.home}</span>
                    <span className="text-[9px] text-slate-500 truncate block">{match.homeOwner}</span>
                </div>
            </div>

            {/* [VS / SCORE] */}
            <div className="flex flex-col items-center justify-center w-[30%] pt-3">
                {isCompleted ? (
                    <div className="flex items-center gap-2 text-3xl font-black italic text-white tracking-tighter">
                        <span className={Number(match.homeScore)>Number(match.awayScore)?'text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.6)]':''}>{match.homeScore}</span>
                        <span className="text-slate-700 text-xl">:</span>
                        <span className={Number(match.awayScore)>Number(match.homeScore)?'text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.6)]':''}>{match.awayScore}</span>
                    </div>
                ) : (
                    <div className="flex flex-col items-center gap-1">
                        <span className="text-[10px] text-slate-600 font-bold bg-slate-900 px-2 py-0.5 rounded-full">VS</span>
                        <div className="h-4 flex items-center">
                            {prediction.hRate >= 60 && <span className="text-[10px] text-emerald-500 font-bold animate-pulse">◀ 우세</span>}
                            {prediction.aRate >= 60 && <span className="text-[10px] text-blue-500 font-bold animate-pulse">우세 ▶</span>}
                        </div>
                    </div>
                )}
            </div>
            ...
            <div className="flex flex-col items-center w-[35%] gap-1">
                {/* 1. 엠블럼 */}
                <img src={match.awayLogo} className="w-12 h-12 rounded-full bg-white object-contain p-1 shadow-lg mb-1" alt="" onError={(e)=>{e.currentTarget.src=FALLBACK_IMG}}/>
                ...
                {/* 3. 팀 정보 */}
                <div className="text-center w-full mt-1">
                    <span className="text-xs font-bold text-white leading-tight truncate w-full block">{match.away}</span>
                    <span className="text-[9px] text-slate-500 truncate block">{match.awayOwner}</span>
                </div>
            </div>
        </div>
        ...
        {/* 상세 기록 (종료된 경기) */}
        {isCompleted && (
            <div className="border-t border-slate-800 pt-2 mt-3 grid grid-cols-[1fr_auto_1fr] gap-2 text-[9px] items-center animate-in fade-in">
        ...
        {match.home !== 'TBD' && match.home !== 'BYE' && match.away !== 'TBD' && (
            <div className="w-full mt-4 mb-2 px-1">
                <div className="text-center text-[8px] text-slate-500 font-bold mb-1 tracking-widest uppercase flex justify-center items-center gap-1">
                    <span>WIN RATE</span>
                    <span className="w-full h-[1px] bg-slate-800"></span>
                </div>
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-emerald-400 w-8 text-right">{prediction.hRate}%</span>
                    <div className="relative flex-1 h-3 bg-slate-800 flex items-center justify-center overflow-hidden rounded-full border border-slate-700 shadow-inner">
                        <div style={{ width: isLoaded ? `${prediction.hRate}%` : '0%' }} className="h-full bg-gradient-to-r from-emerald-800 to-emerald-400 transition-all duration-1000 ease-out absolute left-0 top-0" />
                        <div style={{ width: isLoaded ? `${prediction.aRate}%` : '0%' }} className="h-full bg-gradient-to-l from-blue-800 to-blue-400 transition-all duration-1000 ease-out absolute right-0 top-0" />
                        
                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-20 flex items-center justify-center">
                            <div className="w-4 h-4 bg-slate-950 border border-slate-600 rounded-full flex items-center justify-center shadow">
