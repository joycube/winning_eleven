...
export default function L_PostEditor({ user, owners, viewMode, setViewMode, editingPostId, setEditingPostId, posts, setSelectedPostId }: any) {
    ...
    useEffect(() => {
        if (viewMode === 'EDIT' && editingPostId) {
            ...
            if (post) setPostForm({ title: post.title, content: post.content, cat: post.cat, imageUrl: post.imageUrl || '', youtubeUrl: post.youtubeId ? `https://youtube.com/watch?v=${post.youtubeId}` : '' });
        }
    ...
    return (
        <div className="bg-[#0f172a] rounded-3xl border border-slate-800 shadow-2xl p-6 sm:p-8 animate-in slide-in-from-bottom-4">
            ...
            <div className="space-y-3">
                ...
                <textarea placeholder="자유롭게 소통해 보세요!" className="w-full h-64 bg-slate-900 text-slate-200 p-4 rounded-xl border border-slate-700 text-[13px] outline-none focus:border-emerald-500 resize-none shadow-inner" value={postForm.content} onChange={e => setPostForm({...postForm, content: e.target.value})}></textarea>
