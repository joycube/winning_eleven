        ...
        <div className={`bg-[#0b0e14] p-6 rounded-[2.5rem] border relative transition-all duration-300 ${isLocked ? 'border-slate-800 bg-[#05070a]' : 'border-slate-800'}`}>
                    ...
                    <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                        {waitingPool.map(t => (
                            <TeamCard 
                                key={t.id} 
                                team={t} 
                                draggable={!isLocked} // 🔥 잠금 시 드래그 원천 봉쇄
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
                ...
                {Array.from({ length: bracket.length / 2 }).map((_, mIdx) => (
                    <div key={mIdx} className={`space-y-4 p-5 rounded-3xl border transition-all ${isLocked ? 'bg-black/20 border-slate-800/30' : 'bg-slate-900/20 border-slate-800/50'}`}>
                        ...
                        {[mIdx * 2, mIdx * 2 + 1].map((slotIdx) => (
                            <div 
                                ...
                                className={`relative h-16 rounded-2xl border-2 flex items-center justify-center transition-all group ${
                                    isLocked 
                                    ? 'border-slate-800/50 bg-black/20 cursor-default' // 잠금 시 스타일
                                    : bracket[slotIdx] 
                                        ? 'border-emerald-500/30 bg-emerald-900/10 hover:border-red-500/50 hover:bg-red-950/20 border-dashed cursor-pointer' 
                            ...
                            >
                                {bracket[slotIdx] ? (
                                    <div className="flex items-center gap-4 w-full px-5">
                                        <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center p-1.5 shadow-md shrink-0"><img src={bracket[slotIdx]?.logo} className="w-full h-full object-contain" alt="" onError={(e:any)=>e.target.src=FALLBACK_IMG} /></div>
                                        <div className="flex flex-col flex-1 min-w-0">
                                            <span className={`text-xs font-black italic truncate ${isLocked ? 'text-slate-400' : 'text-white'}`}>{bracket[slotIdx]?.name}</span>
                                            <span className="text-[9px] text-emerald-600 font-bold uppercase truncate">{bracket[slotIdx]?.ownerName}</span>
                                        </div>
                                        {/* 잠금 아닐 때만 삭제 버튼 노출 */}
                                    ...
                                    <div className="flex items-center gap-2">
                                        {isLocked ? (
                                            // 🔥 잠금 상태인데 빈 슬롯이면 'BYE'로 표시 (헷갈림 방지)
                                            <span className="text-[10px] font-black italic text-slate-700 tracking-widest">BYE (PASS)</span>
                                        ) : (
